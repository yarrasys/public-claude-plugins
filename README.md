...
| Plugin | What it does |
|---|---|
| [**yarradev-board**](./yarradev-board) | An agent-native SDLC orchestrator — a reconciliation-loop skill + role subagents (designer → developer → tester, plus a security-advisor and a human production gate) that drive a yarradev HTTP board through a gated lifecycle, running on **your own Claude subscription**. See its [README](./yarradev-board/README.md). |
